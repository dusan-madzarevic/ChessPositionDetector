...
def hough_line(edges, min_line_length=100, max_line_gap=10):
    ...
    for line in lines:
        ...
        cv2.line(edges, pt1, pt2, (37,76, 36), 3, cv2.LINE_AA)
...
def write_crop_images(img, points, img_count, folder_path='./raw_data/'):
    ...
    for row in num_list:
        for s in row:
            ...
            classic = img[start_y: end_y, start_x: end_x]

            gray = cv2.cvtColor(classic, cv2.COLOR_BGR2GRAY)

            kernel = np.array([[0, -1, 0],
                               [-1, 5, -1],
                               [0, -1, 0]])
            image_sharp = cv2.filter2D(src=gray, ddepth=-1, kernel=kernel)
            eq = cv2.equalizeHist(image_sharp)
            eq_crop_half = eq[int(eq.shape[0]/2) - int(eq.shape[0] / 5):int(eq.shape[0]), 0:int(eq.shape[1])]
            eq_crop = eq[int(eq.shape[0] / 2) + int(eq.shape[0] / 4):int(eq.shape[0]), 0:int(eq.shape[1])]
            img_count += 1
            if not ((start_x == end_x) and (start_y == end_y)):
                cv2.imwrite('./raw_data/alpha_data_image' + str(img_count) + '.jpeg', eq)
                cv2.imwrite('./raw_data_base/alpha_data_image' + str(img_count) + '.jpeg', eq_crop)
                cv2.imwrite('./raw_data_crop/alpha_data_image' + str(img_count) + '.jpeg', eq_crop_half)
